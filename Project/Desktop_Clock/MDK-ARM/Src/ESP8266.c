...
#define GPIO_PIN_RST		GPIO_PIN_8		// 复位 GPIO_PIN_X
...
{
	//填写对应的appkey=？&sign=？
	HAL_UART_Transmit(&huart1, "AT", sizeof("AT"), 0xff);
	vTaskDelay(pdMS_TO_TICKS(10));
	AT_Send("GET http://api.k780.com:88/?app=life.time&appkey=74037&sign=c9228380d8d43a8b696406c77902ca87&format=json&HTTP/1.1\r\n", SEND_DATA_STATE);
...
void Time_Keeping(void)
{
	if (Time_Count >= 1000)
	{
		...
		if (Time_HMS[3] == '0' || Time_HMS[3] == '3')		// 每隔30min更新一次时间
		{
			if (Time_HMS[4] == '0')
			{
				ESP8266_GetTime();								// 重新获取时间
				for (uint8_t i = 0; i < 5; i++)		// 补偿五秒
				{
					Time_HMS[7]++;
					if (Time_HMS[7] == ':')
					{
