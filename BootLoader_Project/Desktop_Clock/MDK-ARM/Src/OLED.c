...
#define SCL GPIO_PIN_8		//SCL: 接线时插的口
#define SDA GPIO_PIN_9		//SDA: 接线时插的口
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteData_Inverse(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
...
/**
  ...
  *           故调用显示函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_UpdateArea_Inverse(uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	uint8_t j;
	...
	for (j = Y / 8; j < (Y + Height - 1) / 8 + 1; j ++)
	{
		...
		OLED_SetCursor(j, X);
		/*连续写入Width个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteData_Inverse(&OLED_DisplayBuf[j][X], Width);
	}
}
